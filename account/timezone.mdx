...
| Reports - check-in/check-out columns | Yes |
...
| Business model `activation_date` | Stored as a date, not a timestamp - timezone-agnostic. |
| Audit date range pickers | The pickers select calendar dates regardless of timezone. |
| CSV/PDF exports | Exports use the user's display timezone at the time of export. |
