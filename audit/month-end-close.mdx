...
**Month-End Close** runs both of PX's checks over a single month and puts the combined result on one page: the Financial Audit (fees, taxes, and commission) and the Accounting Audit (ledger and guest-money integrity). Use it as the monthly ritual you hand to your accountant.
...
Reservation and folio integrity issues for the month, with counts for unresolved, fix in progress, found, and fixed. The list defaults to **open** issues so it shows outstanding work.

You can mark issues fixed or dismissed from this tab. Doing so removes them from the exception counts and the money flagged for this close, so the headline banner and the Overview figures update as you work.
