...
<Frame caption="Assign properties dialog">
  ![Assign properties](/images/screenshots/business-models-assign.png)
</Frame>
...
Business models have an `activation_date`. PX uses this date plus the property's assignment record to decide which model applies to a given reservation:
...
If you edit an active model's `pmc_config`, PX creates a new version automatically. The version bump triggers a diagnostics re-run for every property assigned to the model so existing findings are recomputed against the new rules.
