...
Editing the `pmc_config` of an active business model creates a **new version** of the model automatically. Old versions are immutable, so:
...
- A `parent_id` chains versions together so you can browse history.
...
| `activation_date` | Yes (to run audits) | The date this model becomes effective. Earlier reservations use the prior model. |
| `status` | Yes | draft / active / inactive / archived. |
| `version` | Auto | Increments on each meaningful edit. |
| `pmc_config.net_rental_income` | No | Income items, expense items, excluded items. |
| `pmc_config.commission_setup` | No | Confirmed, canceled, and minimum commission rules. |
| `pmc_config.trust_account_setup` | No | Recurring income and expense charges. |
| `assigned_listings_count` | Auto | Number of properties using this model. |
