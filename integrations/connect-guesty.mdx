...
PX has read-only access. It never writes back to Guesty.

### Properties

- Listing ID
- Name (nickname first, then title)
- Full address (street, city, state, postal code, country)
- Bedrooms, bathrooms, accommodates (max guest count)
- Currency
- Cleaning fee (account-level default)
- Active / inactive status
- Thumbnail picture

### Owners

Guesty's API does not expose owner records, so PX does not pull owners from Guesty. Owner mapping is managed directly in PX.

### Reservations

- Reservation ID and listing reference
- Status (Confirmed, Pending, Canceled, Inquiry - mapped from Guesty's status field)
...
- Cancellation: canceled date, canceled-by, cancellation reason, cancellation policy

### Per-charge financial data

- Per-charge amount, type (accommodation, fee, expense, tax), description, taxable flag
- Per-night accommodation breakdown (base price per night)
- Per-reservation aggregates: fare accommodation, fare cleaning, total taxes, subtotal, host service fee, net income

### Tax configuration

PX reads taxes directly from Guesty (account-level + per-listing) - rates, applicability, and metadata. Tax Configuration is a live mirror; changes you make in Guesty appear in PX after clicking **Refresh from PMS**. Channel sync overrides and Airbnb tax remittance are then layered on top inside PX.

### Webhook events

PX subscribes to these Guesty webhooks:

- `listing.new`, `listing.updated`, `listing.removed` - re-fetches the listing.
- `reservation.new`, `reservation.updated` - re-fetches the reservation.
- `payments.*` (received, failed, refunded, overdue, dispute, authentication, etc.) - acknowledged. Financial data is read from the reservation snapshot, not the webhook payload, so a missed payment webhook never causes PX to lose data.
