...
1. Sign in to Guesty.
2. Go to **Integrations** -> **API** (or **Settings** -> **Integrations** depending on your Guesty plan).
3. Create a new application named something like `PX Accounting`.
4. Copy the generated **Client ID** and **Client Secret**. You will paste them into PX next.

The credentials should grant read access to listings, reservations, and accounting data. PX never writes back to Guesty.
...
2. Click **Sync from Guesty**.

PX fetches your full listing catalog and creates a row per listing. New properties get optimization tracking; existing properties update in place. Re-syncing later is safe - PX matches on Guesty's listing ID.

## What PX reads from Guesty

| Data | When fetched |
|---|---|
| Listings | On sync (Properties page) |
| Listing config: fees, taxes, channel commissions | On demand (PMS Configuration page) |
| Reservations | On demand (property detail Reservations tab, Reports, Audits) |
| Journal entries (per reservation) | On demand (folio modal, Audits) |
| Owners | On owner sync |

PX caches Guesty journal entries for 30 days. They auto-purge after that to keep your data fresh.
