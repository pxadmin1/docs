...
<Note>
  This wizard is currently available for **Guesty** accounts only. Other PMSes handle marketplace remittance through different mechanisms today; broader PMS support is on the roadmap. The setting is stored in PX only - PX does not push it back to your PMS or to Airbnb. It exists purely to inform PX's audit logic.
</Note>
...
A single yes/no toggle: **`airbnb_remits`**.
