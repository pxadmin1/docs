...
2. **Sync behavior per channel** - the same tax may sync to your direct channel but not to Booking.com because Booking.com cannot pass through the rate. Not every PMS exposes this layer in its API, so PX provides a channel override layer you maintain in PX itself.
